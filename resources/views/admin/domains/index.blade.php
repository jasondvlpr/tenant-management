    ...
    <div x-data="{
        ...
        <div x-show="openModalAlias" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-xs" style="display: none;">
            <div @click.outside="openModalAlias = false" x-transition.scale.95 class="w-full max-w-lg overflow-hidden rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800">
                ...
                <form action="{{ route('domains.store') }}" method="POST" class="mt-5 space-y-4" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                    @csrf
                    <div>
                    ...
                    <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                        ...
                        <button type="submit" :disabled="isSubmitting" :class="isSubmitting ? 'opacity-75 cursor-wait' : 'hover:bg-amber-600'" class="rounded-xl bg-amber-500 px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-amber-500/20 transition flex items-center justify-center min-w-[180px]">
                            <svg x-show="isSubmitting" style="display: none;" class="mr-2 h-4 w-4 animate-spin text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            <span x-text="isSubmitting ? 'Sinkronisasi...' : 'Daftarkan & Sync DNS'">Daftarkan & Sync DNS</span>
                        </button>
                    </div>
                </form>
            </div>
