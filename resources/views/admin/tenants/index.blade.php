    ...
    <div x-data="{ 
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button @click="openNSManager(item)" class="inline-flex items-center gap-1 rounded-xl bg-orange-50 px-2.5 py-1.5 text-xs font-bold text-orange-600 hover:bg-orange-100 dark:bg-orange-500/10 dark:text-orange-400 dark:hover:bg-orange-500/20 transition shadow-xs" title="Lihat Name Server (NS) & Cek Koneksi">
                                            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z" /></svg>
                                            <span>Status NS</span>
                                        </button>
                                        <button @click="openDomainManager(item)" class="inline-flex items-center gap-1 rounded-xl bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700/80 transition" title="Kelola DNS & Domain Alias">
        ...
        <div x-show="openModalCreate" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-xs" style="display: none;">
            <div @click.outside="openModalCreate = false" x-transition.scale.95 class="w-full max-w-xl overflow-hidden rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800">
                ...
                <form action="{{ route('tenants.store') }}" method="POST" class="mt-5 space-y-4">
                    @csrf
                    ...
                    <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                        ...
                        <button type="submit" class="rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-500/20 hover:bg-indigo-500 transition">Eksekusi API Deploy & DNS</button>
                    </div>
                </form>
            </div>
        </div>
